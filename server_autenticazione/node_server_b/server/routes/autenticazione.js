...
router.post('/avviaAutenticazione', async (req, res) => {
    ...
    if (encryptedDumpA !== false && encryptedDumpB !== false){
        ...
        const args = `${decryptedDumpA} ${decryptedDumpB} ${helper_dataA} ${helper_dataB} ${nonceA} ${nonceB} ${id_dispositivoA} ${id_dispositivoB} ${chiaveA} ${chiaveB}`;
        const {stdout} = await executeCProgram("cd server/c_code/ && ./genera_M_e_PRK",`${args}`);

        let responseData = JSON.parse(stdout);

        const indirizzo_serverB = "127.0.0.1:3008";
        ...
        if (success){
            ...
            if (find) {
                ...
                if (encryptedDump !== false){
                    ...
                    // da inviare al file c i due dump, i due id, ninita e ninitb
                    const args = `${decryptedDump} ${helper_data} ${id} ${NInita} ${NSA} ${NSB} ${M_SA} ${digest}`;
...
// Endpoint per verificare l'autenticazione da parte del server B per il dispositivo B
...
router.post('/confermaAutenticazioneDispositivoB', async (req, res) => {
    ...
    if(stdout.trim() === "true"){
        ...
        // Avvio il processo di autenticazione e salvo le informazioni del dispositivo B
